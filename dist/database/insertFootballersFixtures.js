...
export const insertFootballersFixtures = async () => {
    try {
        const rawData = fs.existsSync(RAW_FOOTBALLERS_FILE)
        ...
        const fixtureIdsToDelete = [...existingFixtureIds].filter((id) => !uniqueFixtures.has(id));
        ...
        await Promise.all(fixturesToInsert.map(async (fixture) => {
        ...
        await Promise.all(Object.entries(rawData).flatMap(([footballerId, footballer]) => footballer.fixtures.map(async (fixture) => {
        ...
        console.error("Couldn't populate the fixtures table. Error:", error?.message);
    }
    finally {
