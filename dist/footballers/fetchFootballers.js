...
export const fetchFootballers = async () => {
    try {
        ...
        for (let i = 0; i < footballerIds.length; i += BATCH_SIZE) {
            ...
            console.log(`Processing batch: ${i + 1} - ${Math.min(i + BATCH_SIZE, footballerIds.length)}`);
            let retries = 0;
            let success = false;
            while (retries < MAX_RETRIES && !success) {
                try {
                    await Promise.all(batch.map(async (footballerId) => {
                        const footballer = await getFootballer(footballerId);
                        if (!footballer) {
                    ...
                    fs.writeFileSync(RAW_FOOTBALLERS_FILE, JSON.stringify(rawData, null, 2));
                    console.log(`Batch ${i + 1} - ${i + BATCH_SIZE + 1} saved to raw data file.`);
                    // Batch succeeded, break out of retry loop
                    success = true;
                }
                catch (error) {
                    retries++;
                    console.error(`Error processing batch ${i + 1}. Retry ${retries}/${MAX_RETRIES}. Error: ${error.message}`);
                    if (retries >= MAX_RETRIES) {
                        console.error(`Batch ${i + 1} failed after ${MAX_RETRIES} retries. Exiting process.`);
        ...
        console.error(`There was an error trying to fetch footballers: ${error.message}`);
        return;
    }
