...
export const getSamplePlayers = (totalPlayers: number, blacklist: Set<number>): number[] => {
  ...
  const eligiblePlayers = Array.from({ length: maxSampleId }, (_, i) => i + 1).filter(
    (id) => !blacklist.has(id)
  );

  // Split eligible players into two groups
  const top30PercentPlayers = eligiblePlayers.filter((id) => id >= top30PercentStart);
  const remainingPlayers = eligiblePlayers.filter((id) => id < top30PercentStart);
  ...
  const sampleTop30 = shuffledTop30.slice(0, 50000); // 50k from top 30%
  const sampleRemaining = shuffledRemaining.slice(0, 50000); // 50k from below 60%
...
export const readBlacklist = (): Set<number> => {
  ...
  const blacklist = fs.readFileSync(BLACKLIST_FILE, "utf8").split("\n").map((line) => parseInt(line.split(",")[0], 10));
