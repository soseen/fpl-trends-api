import { CSV_FILE, LAST_GAMEWEEK_FILE, RAW_PLAYERS_ALL_FILE, readCheckpoint, writeCheckpoint, } from "./file.helpers";
import fs from "fs";
...
export const fetchAllRawData = async () => {
    ...
    const playerIds = Array.from({ length: totalPlayers }, (_, i) => i + 1).filter((id) => !blacklist.has(id));
    ...
    for (let i = 0; i < remainingPlayers.length; i += BATCH_SIZE) {
        ...
        try {
            ...
            console.error("Error occurred while processing batch. Aborting...", error);
